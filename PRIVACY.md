...
_Last updated / 最後更新：2026-06-19_
...
- **Local only.** All behaviour happens inside your browser tab on `claude.ai`. Your conversations stay between you and Claude (Anthropic), exactly as they would without this extension.
...
You can verify all of the above by reading the source — it is fully open and only 483 lines of `content.js`.
...
- **純本機運作。** 所有行為都發生在你瀏覽器的 `claude.ai` 分頁內。你的對話一如往常只在你與 Claude（Anthropic）之間。
...
以上每一點你都能透過閱讀原始碼自行驗證 —— 全部開源，`content.js` 只有 483 行。
